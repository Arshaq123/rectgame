// createSprite(),isTouching(), 

//yes or no - boolean - true(1) or false(0)

//arguments or parameters
//sprite1- movingrect, sprite2 - rect1
function isTouching(sprite1 , sprite2){

    //lets code to write a logic to check if two objects are touching each other
    if(sprite1.x - sprite2.x < (sprite2.width + sprite1.width)/2 &&
    sprite2.x- sprite1.x < sprite2.width/2 + sprite1.width/2 &&
    sprite1.y - sprite2.y < sprite2.height/2 + sprite1.height/2&&
    sprite2.y- sprite1.y < sprite2.height/2 + sprite1.height/2 ){

        return true;

    }

    else {

        return false;

    }




}

function BounceOff(sprite1,sprite2){

  //rect1
  if(sprite1.x - sprite2.x < (sprite2.width + sprite1.width)/2 &&
    sprite2.x- sprite1.x < sprite2.width/2 + sprite1.width/2)
  {
    //if two sprites are colliding then bounce them off
    //by reversing their velocity
     sprite1.velocityX = sprite1.velocityX * (-1);// before colliding = 5, after colliding = -5
     sprite2.velocityX = sprite2.velocityX * (-1);
  }


  if(sprite1.y - sprite2.y < sprite2.height/2 + sprite1.height/2&&
  sprite2.y- sprite1.y < sprite2.height/2 + sprite1.height/2 )
  {

    sprite1.velocityY= sprite1.velocityY * (-1);
    sprite2.velocityY = sprite2.velocityY * (-1);
  }

}